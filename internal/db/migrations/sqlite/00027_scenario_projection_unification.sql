...
-- Scenario is now the common projection identity. SQLite needs a table
-- rebuild because the old CHECK only knew about standalone and payable-plan
-- scenarios. Existing standalone rows start inactive to preserve the old
-- explicit-selection behavior; plans stay visible by default.
PRAGMA foreign_keys = OFF;

CREATE TABLE scenarios_new (
    id                   TEXT PRIMARY KEY,
    kind                 TEXT NOT NULL CHECK (kind IN ('debt_plan', 'receivable_plan', 'standalone', 'recurring')),
    name                 TEXT NOT NULL,
    payable_id           TEXT REFERENCES payables (id) ON DELETE CASCADE,
    is_active            INTEGER NOT NULL DEFAULT 1 CHECK (is_active IN (0, 1)),
    is_accounting_source INTEGER NOT NULL DEFAULT 0 CHECK (is_accounting_source IN (0, 1)),
    created_at           TEXT NOT NULL,
    updated_at           TEXT NOT NULL,
    CHECK (is_accounting_source = 0 OR kind IN ('debt_plan', 'receivable_plan')),
    CHECK (
...
INSERT INTO scenarios_new (
    id, kind, name, payable_id, is_active, is_accounting_source, created_at, updated_at
)
SELECT
    s.id,
    s.kind,
    s.name,
    s.payable_id,
    CASE WHEN s.kind = 'standalone' THEN 0 ELSE 1 END,
    CASE
        WHEN s.kind IN ('debt_plan', 'receivable_plan')
         AND s.id = (
             SELECT older.id
             FROM scenarios older
...
CREATE UNIQUE INDEX uq_scenarios_accounting_source
    ON scenarios (payable_id)
    WHERE is_accounting_source = 1;

-- Every Payable gets an accounting scenario. Existing plans are reused when
-- present; a payable that never had a plan receives a new identity, so no
-- legacy id is ever assumed to be a scenario id.
INSERT INTO scenarios (id, kind, name, payable_id, is_active, is_accounting_source, created_at, updated_at)
SELECT
    lower(printf('%08x-%04x-4%03x-a%03x-%012x', abs(random()) % 4294967296, abs(random()) % 65536, abs(random()) % 4096, abs(random()) % 4096, abs(random()) % 281474976710656)),
...
CREATE INDEX idx_scenario_recurring_schedules_category_id
    ON scenario_recurring_schedules (category_id);
CREATE INDEX idx_scenario_recurring_schedules_account_id
    ON scenario_recurring_schedules (account_id);

-- Keep the old table/API alive during the transition, but give every old
-- reconciliation its new scenario identity as well.
...
SELECT r.id, lower(printf('%08x-%04x-4%03x-a%03x-%012x', abs(random()) % 4294967296, abs(random()) % 65536, abs(random()) % 4096, abs(random()) % 4096, abs(random()) % 281474976710656))
...
SELECT
    m.scenario_id, r.kind, r.amount, r.category_id, r.account_id, r.cadence,
...
UPDATE recurrence_reconciliations
SET scenario_id = (
    SELECT m.scenario_id FROM recurring_commitment_scenario_map m
    WHERE m.recurring_commitment_id = recurrence_reconciliations.recurring_commitment_id
);
CREATE INDEX idx_recurrence_reconciliations_scenario
    ON recurrence_reconciliations (scenario_id, occurrence_date);

-- Rebuild actions so a new rule can target a recurring Scenario without
-- manufacturing a legacy recurring_commitment_id. Existing targets are
-- copied to both columns and remain usable by the compatibility API.
...
SELECT
    ...
    CASE WHEN a.action_type = 'reconcile' THEN (
        SELECT m.scenario_id FROM recurring_commitment_scenario_map m
...
CREATE INDEX idx_automation_rule_actions_scenario_id
...
-- One generic relation table represents payable settlements, plan
-- allocations, and recurring occurrence reconciliation. The partial unique
-- transaction index is the database guard against satisfying two complete
-- events with the same real transaction; allocations remain many-to-many.
...
CREATE INDEX idx_scenario_realizations_scenario_occurrence
    ON scenario_realizations (scenario_id, occurrence_date);
CREATE INDEX idx_scenario_realizations_transaction_event
    ON scenario_realizations (scenario_transaction_id, transaction_id);
CREATE INDEX idx_scenario_realizations_transaction_settlement_reconciliation
    ON scenario_realizations (transaction_id)
    WHERE relation_type IN ('settlement', 'reconciliation');
CREATE UNIQUE INDEX uq_scenario_realizations_reconciliation_event
    ON scenario_realizations (scenario_id, occurrence_date)
    WHERE relation_type = 'reconciliation';
CREATE UNIQUE INDEX uq_scenario_realizations_one_complete_event_transaction
    ON scenario_realizations (transaction_id)
    WHERE transaction_id IS NOT NULL
      AND relation_type IN ('settlement', 'reconciliation')
      AND state = 'linked';

-- Backfill the generic table without changing the legacy rows. This makes
-- the migration reversible at the application layer and lets old endpoints
-- continue serving while new readers use scenario_realizations.
INSERT INTO scenario_realizations (
    id, scenario_id, transaction_id, relation_type, state, origin, linked_amount, created_at
)
SELECT
    lower(printf('%08x-%04x-4%03x-a%03x-%012x', abs(random()) % 4294967296, abs(random()) % 65536, abs(random()) % 4096, abs(random()) % 4096, abs(random()) % 281474976710656)), s.id, l.transaction_id, 'settlement', 'linked', 'manual', l.linked_amount, l.linked_at
FROM payable_transaction_links l
JOIN scenarios s ON s.payable_id = l.payable_id AND s.is_accounting_source = 1;

INSERT INTO scenario_realizations (
    id, scenario_id, scenario_transaction_id, transaction_id, relation_type, state, origin, allocated_amount, created_at
)
SELECT
    lower(printf('%08x-%04x-4%03x-a%03x-%012x', abs(random()) % 4294967296, abs(random()) % 65536, abs(random()) % 4096, abs(random()) % 4096, abs(random()) % 281474976710656)), st.scenario_id, str.scenario_transaction_id,
    l.transaction_id, 'allocation', 'linked', 'manual', str.allocated_amount, str.created_at
...
INSERT INTO scenario_realizations (
    id, scenario_id, occurrence_date, transaction_id, relation_type, state, origin, created_at
)
SELECT
    lower(printf('%08x-%04x-4%03x-a%03x-%012x', abs(random()) % 4294967296, abs(random()) % 65536, abs(random()) % 4096, abs(random()) % 4096, abs(random()) % 281474976710656)), rr.scenario_id, rr.occurrence_date, rr.transaction_id,
