...
-- That ordering is what lets this migration run with foreign keys enforced
-- and inside goose's transaction, so a failure rolls the whole thing back
-- instead of leaving a half-rebuilt schema behind.
...
-- them — the filter below is a floor, not an expected path.
UPDATE automation_rule_actions
SET scenario_id = (
    SELECT m.scenario_id FROM recurring_commitment_scenario_map m
    WHERE m.recurring_commitment_id = automation_rule_actions.recurring_commitment_id
)
WHERE action_type = 'reconcile' AND scenario_id IS NULL AND recurring_commitment_id IS NOT NULL;
...
-- A reconcile action with no scenario left to point at cannot be carried
-- over: the new CHECK forbids it, and there is nothing for the resolver to
-- key on. The old CHECK plus the commitment FK already made this
-- unreachable; it is filtered rather than trusted so the rebuild cannot
-- fail halfway through.
INSERT INTO automation_rule_actions_new (id, rule_id, action_type, scenario_id, category_id, position)
SELECT id, rule_id, action_type, scenario_id, category_id, position
FROM automation_rule_actions
WHERE action_type != 'reconcile' OR scenario_id IS NOT NULL;
...
-- the legacy alias is missing). Rolling forward again re-derives nothing:
-- the Up above only materializes commitments that still exist. Check for
-- schedules with a NULL category_id before relying on this Down.
...
UPDATE automation_rule_actions
SET recurring_commitment_id = (
    SELECT m.recurring_commitment_id FROM recurring_commitment_scenario_map m
    WHERE m.scenario_id = automation_rule_actions.scenario_id
)
