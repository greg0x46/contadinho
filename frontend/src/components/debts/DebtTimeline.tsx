...
function realizationLabel(realization: Realization, links: DebtLinkedTransaction[]): string {
  const link = links.find((candidate) => candidate.id === realization.debt_link_id);
...
function AccumulatedDeviationBanner({ value }: { value: string }) {
  ...
  if (isOnTrack) {
    return <Alert type="success" showIcon message="Em dia com o plano de pagamento até hoje." />;
  }
  return (
    <Alert
...
type TimelineEntry =
  ...
  | { kind: "link"; date: string; link: DebtLinkedTransaction };

// DebtTimeline replaces what used to be two separate tabs ("Plano de
// pagamento" and "Transações vinculadas"): a debt's installments and its
// linked transactions are one story, not two — an installment is "paid" by
...
// parcela unset still links standalone, exactly like before this existed.
export function DebtTimeline({
  ...
  isSearching: boolean;
  isLinking: boolean;
  onLinkTransaction: (transactionId: string) => Promise<DebtLink>;
  ...
  const unlink = async (link: DebtLinkedTransaction) => {
  ...
  const allocateExistingLink = async (linkId: string, installmentId: string, amount: number) => {
    try {
      await plan.allocateRealization({
        ...
        write: { debt_link_id: linkId, allocated_amount: amount },
      });
    } catch (error) {
    ...
    try {
      ...
      if (selectedInstallmentId !== "none") {
        await plan.allocateRealization({
          ...
          write: { debt_link_id: link.id, allocated_amount: Number(link.linked_amount) },
        });
      }
  ...
  const allocatedLinkIds = new Set(installments.flatMap((i) => i.realizations.map((r) => r.debt_link_id)));
                    ...
                    {installment.realizations.map((realization) => (
                      ...
                      >
                        ...
                        <Popconfirm
                          ...
                          description="A parcela deixa de contar esse valor como pago; a transação em si permanece vinculada à dívida."
