...
function draftFrom(debt: Debt | null): Draft {
  return debt
    ? { name: debt.name, totalAmount: Number(debt.total_amount), initialRemainingAmount: null }
    : blankDraft;
}
...
export function DebtForm({
  ...
  const [draftState, setDraftState] = useState<Draft>(() => draftFrom(debt));
  const [error, setError] = useState<string | null>(null);
  const isEditing = debt !== null;

  useEffect(() => {
    if (open) {
      setDraftState(draftFrom(debt));
      setError(null);
    }
  }, [open, debt]);
  ...
  const submit = () => {
    if (draftState.name.trim() === "") {
      setError("Informe um nome para a dívida.");
      return;
    }
  ...
  return (
    <Drawer
      title={isEditing ? "Editar dívida" : "Nova dívida"}
      open={open}
      onClose={onCancel}
      width={420}
      destroyOnHidden
    ...
    >
      <Flex vertical gap="middle">
        ...
        <div className="filter-field">
          <label htmlFor="debt-total-amount">Valor total</label>
          <InputNumber
        ...
        {!isEditing && (
          <div className="filter-field">
            <label htmlFor="debt-initial-remaining-amount">
              Valor restante inicial (opcional)
            </label>
            <InputNumber
              id="debt-initial-remaining-amount"
              style={{ width: "100%" }}
              min={0}
              step={0.01}
              decimalSeparator=","
