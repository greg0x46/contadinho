...
import { PayableTimeline } from "./PayableTimeline";
...
function baseProps(overrides: Partial<Parameters<typeof PayableTimeline>[0]> = {}) {
  return {
    payableId,
    kind: "debt" as const,
    links: [] as PayableLinkedTransaction[],
    search: "",
...
function renderTimeline(overrides: Partial<Parameters<typeof PayableTimeline>[0]> = {}) {
  const props = baseProps(overrides);
  render(
    <QueryTestProvider>
      <PayableTimeline {...props} />
    </QueryTestProvider>,
  );
  return props;
}
...
describe("PayableTimeline", () => {
  it("links a transaction standalone when no plan exists", async () => {
  ...
  it("shows an installment's allocated realizations and their Desalocar action directly, with no expand step", async () => {
    const link: PayableLinkedTransaction = {
      id: linkId,
  ...
  it("highlights the realized total instead of the planned amount when a payment diverges from the plan", async () => {
    const link: PayableLinkedTransaction = {
      id: linkId,
      transaction_id: "99999999-9999-4999-8999-999999999999",
      occurred_at: "2026-07-10T12:00:00Z",
    ...
    const heading = await screen.findByText(/Parcela 1\/25/);
    const headerLine = heading.closest("span") as HTMLElement;
    expect(within(headerLine).getByText(/R\$\s*350,00/).tagName).toBe("STRONG");
    expect(within(headerLine).getByText(/planejado R\$\s*200,00/)).toBeVisible();
  });
      ...
      transactions: [
        {
          ...
          status: "paga",
          realizations: [
            { id: realizationId, payable_link_id: linkId, allocated_amount: "150.00", created_at: "2026-07-31T12:00:00Z" },
          ],
        },
      ],
    };
